...
import type { RestEndpointMethodt--change--ypes } from '@octokit/rest';
import { argv, repoDetails, github } from './init';
import { getTagToChangelogMap, getTags } from './changelogsAndt--change--ags';
...
async function upda--change--teReleaseNotes() {
  ...
  for (const tag of tags) {
    ...
    const releaseDetails:
      | RestEndpointMethodt--change--ypes['repos']['upda--change--teRelease']['parameters']
      | RestEndpointMethodt--change--ypes['repos']['createRelease']['parameters'] = {
      ...repoDetails,
      ...
      try {
        if (hasBeenReleased) {
          await github.repos.upda--change--teRelease(
            releaseDetails as RestEndpointMethodt--change--ypes['repos']['upda--change--teRelease']['parameters'],
          );
        } else {
          await github.repos.createRelease(
            releaseDetails as RestEndpointMethodt--change--ypes['repos']['createRelease']['parameters'],
          );
        }
