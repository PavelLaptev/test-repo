...
export type Resolvedt--change--ype = {
  type: string;
  name: string;
};

export type RTLiteral = Resolvedt--change--ype & {
  type: 'StringLiteral' | 'NumberLiteral' | 'BooleanLiteral';
  value: string | number | boolean;
};

export type RTUnion = Resolvedt--change--ype & {
  type: 'union';
  types: Resolvedt--change--ype[];
};

export type RTEnum = Resolvedt--change--ype & {
  type: 'enum';
  values: RTLiteral[]; // these should be just
};

export type RTIntersection = Resolvedt--change--ype & {
  type: 'intersection';
  types: Resolvedt--change--ype[];
};

export type RTArray = Resolvedt--change--ype & {
  type: 'array';
  indexedt--change--ype: Resolvedt--change--ype;
};

export type RTObject = Resolvedt--change--ype & {
  type: 'object';
  props: Record<string, Resolvedt--change--ype>;
};
...
export class Parser {
  ...
  private static isLiteral(type: Resolvedt--change--ype): boolean {
    return type.type === 'StringLiteral' || type.type === 'NumberLiteral' || type.type === 'BooleanLiteral';
  }

  // eslint-disable-next-line @typescript-eslint/naming-convention
  private customResolveTypeOverride(typeNode: ts.Type, name: string): Resolvedt--change--ype | undefined {
  ...
  // eslint-disable-next-line @typescript-eslint/naming-convention
  private resolveType(typeNode: ts.Type, depth = 0): Resolvedt--change--ype {
    ...
    const typeMap = {
      [TypeFlags.Object]: () => {
        ...
        // Array
        const numberIndexedt--change--ype = typeNode.getNumberIndexType();
        if (numberIndexedt--change--ype) {
          return {
            type: 'array',
            indexedt--change--ype: this.resolveType(numberIndexedt--change--ype, depth + 1),
            name,
          } as RTArray;
        }
        ...
        typeNode.getProperties().forEach(prop => {
          const childt--change--ype = this.checker.getTypeOfSymbolAtLocation(prop, prop.valueDeclaration!);

          props[prop.getName()] = this.resolveType(childt--change--ype, depth + 1);
        });
        return {
          type: 'object',
          props,
    ...
    if (typeMap[typeNode.flags as keyof typeof typeMap]) {
      ...
      return {
        type: 'intersection',
        types: typeNode.types.map(childt--change--ypeNode => this.resolveType(childt--change--ypeNode, depth + 1)),
        name,
      } as RTIntersection;
    } else if (typeNode.isUnion()) {
      const subTypes = typeNode.types.map(childt--change--ypeNode => this.resolveType(childt--change--ypeNode, depth + 1));
  ...
  public getPropsInfo(propsObj: ts.Symbol, defaultProps: StringIndexedObject<string> = {}): Props {
    ...
    propertiesOfProps.forEach(prop => {
      ...
      const resolvedt--change--ype = this.resolveType(propType);
  ...
  public getFullJsDocComment(symbol: ts.Symbol): JSDoc {
    ...
    tags.forEach(tag => {
      const trimmedt--change--ext = ts.displayPartsToString(tag.text).trim();
      const currentValue = tagMap[tag.name];
      tagMap[tag.name] = currentValue ? `${currentValue}\n${trimmedt--change--ext}` : trimmedt--change--ext;
...
function statementIsStateless(statement: ts.Statement): boolean {
  ...
  for (const child of children) {
    ...
    if (left) {
      ...
      if (name.escapedt--change--ext === 'defaultProps') {
        return true;
      }
    }
  }
  ...
  const [textValue] = source.statements
    ...
    .filter(statement => {
      ...
      return (
        ...
        (expr.left as ts.PropertyAccessExpression).name.escapedt--change--ext === propertyName
      );
    })
    .filter(statement => {
      const expr = (statement as ts.ExpressionStatement).expression as ts.BinaryExpression;

      return ((expr.left as ts.PropertyAccessExpression).expression as ts.Identifier).escapedt--change--ext === exp.getName();
    })
    .filter(statement => {
